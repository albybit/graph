...
{
    int id, visited= 0;
    int parent = -1, isItLeaf = -1, end = -1, subTreeSize= 0;
    struct path pt;
};
struct csc_Node{
    int *csc_nodes;
...
class Graph {
    ...
    int numberOfroots;
    struct rts roots;
    struct rts secondroots;
    struct nodesAP *clusters;
    int ncltrs=0;
   struct leaf_st leafs;
   struct leaf_st secondQueue;
...
public:
  ...
  struct  lf_linked *InsertLeafInlinkedList(int u);
    ...
    void InsertLeafsInArray();

    void InsertRootII(int u);

    struct rts_linked *InsertRootInLinkedListII(int u);
